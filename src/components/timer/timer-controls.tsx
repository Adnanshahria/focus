...
export function TimerControls() {
  const { isActive, start, pause, reset, addTime, subtractTime, endAndSaveSession } = useTimer();
       ...
       {!isActive ? (
         <>
            <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                <Button onClick={endAndSaveSession} size="icon" variant="ghost" className="rounded-full w-14 h-14 text-muted-foreground hover:text-foreground">
                    <RotateCcw />
                </Button>
            </motion.div>
